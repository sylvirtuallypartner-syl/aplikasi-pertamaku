...
import { deleteRewardTier, updateRewardTier } from "@/lib/db";
...
export async function PATCH(
  ...
  try {
    ...
    return NextResponse.json(
      { error: err instanceof Error ? err.message : "Gagal mengubah reward" },
      { status: 500 }
    );
  }
}
...
export async function DELETE(
  ...
  try {
    ...
    return NextResponse.json(
      { error: err instanceof Error ? err.message : "Gagal menghapus reward" },
      { status: 500 }
    );
  }
}
